...
いろんなネタが出てきていますが、どうみても詐欺じみた企画も
たくさん見えてきたので、そろそろまとめておこうかと思います。
...
そうすることにより、「報酬をエサに、充分に多数のマイナーが集まる」ことを目論み、
...
これがないと、極少量の計算資源でブロックチェーンを改ざんすることができるため、
改ざんできない、というブロックチェーンのメリットを享受できません。
...
つまり、完全に信頼できる機材なんて、この世に存在しないので、ここでも、
...
51% 攻撃の名前だけ広まって、「ブロックチェーンは 51% 乗っ取られると改ざんされる」と
理解されていることが多いですが、51% というのは、改ざんの「成功率が 100% になる」条件であって、
改ざんが可能という話であれば、10% でも 20% でも数パーセントの確率で改ざんに成功します。
それが、承認からの経過時間によって信頼度が高まる、という話でしかありません。

ブロックチェーンでは、悪意のあるマイナーが多ければ多いほど（過半数に届かなくとも）、
また、承認からの経過時間が短ければ短いほど、改ざん成功率が高まる仕組みなので、
その反対の条件を満たす必要があるわけですが、
RDB には承認からの経過時間など関係ないので、承認後、即、信頼できます。
プライベートチェーンは経過時間によって信頼度が上がる仕組みなので、待ち時間があります。
不便ですね。

最後にトランザクション、取引履歴の管理ですが、
ブロックチェーンでは改ざん不能、つまり後からの変更ができないため、
Script と呼ばれる仮想マシンを使った大掛かりな仕組みを用意しています。
RDB だったら、過去のトランザクションデータの一部を変更できるので、
...
パブリックチェーンと比べれば、比較的現実的な解決策と言えます。
...
4. Ethrerum のトークンもなんとか現実的です。これは許容できると思います
